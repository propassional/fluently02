from robot_backend import SimRobotBackend, gen_rings_poses, gen_arc_poses, gen_s_poses
...
z_table = 0.92
# imposta il robot per l'utilizzo dei joint
...
rings = gen_rings_poses(obj_pose=obj_pose_ROBOT, radius=0.2, stretch=2)
arc = gen_arc_poses(obj_pose=obj_pose_ROBOT, radius=0.2,stretch=2)
s = gen_s_poses(obj_pose=obj_pose_ROBOT, radius=0.2,stretch=2)

# robot.env.hold() # view the backend
...
for pose in rings:
    c+=1
    suc, q = robot.ik_collision_free((pose),q0=seed_configuration, n_trials=50)
    print(c)
    print(q[0])
