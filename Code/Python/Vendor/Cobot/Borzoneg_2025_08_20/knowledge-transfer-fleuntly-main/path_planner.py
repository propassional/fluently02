...
from typing import Tuple # Added by MB
from typing import List # Added by MB
import numpy as np # Added by MB
...
class Box():
    #def __init__(self, centre: tuple[float, float], length:float, width: float, heigth: float): # Original
    def __init__(self, centre: Tuple[float, float], length:float, width: float, heigth: float): # Modified by MB
        self.c = centre
        self.l = length
        self.w = width
        self.h = heigth
...
class SimRobotBackend(ERobot):
    ...
    #def set_joint_limit(self, joint_index: int, limit: tuple[float, float]):  # Original
    def set_joint_limit(self, joint_index: int, limit: Tuple[float, float]): # Modified by MB
    ...
    def set_geom_free_space(self, axis: chr, free_space: Tuple[float, float]): # Modified by MB
        if axis == 'x':
    ...
    def find_all_ik_solutions(self, Tep: sm.SE3, n_sols=10, tfms:List[sm.SE3]=[sm.SE3([0,0,0])], tol=1e-6, mask=None): # MB
        T = copy.deepcopy(Tep)
        T = (T * self.tcp_frame_transf.inv())
        solutions = []
    ...
    def generate_continous_trajectory_jtraj(self, poses: List[np.ndarray], q0=None, speed=0.05, t_step=200,
                                            remove_spikes=False) -> Tuple[bool, np.ndarray, Tuple[int, int]]: # MB
        j_confs = []
        traj_q = []
        success = False
    ...
    def plot_traj(self, traj: List[ndarray], loop=False, hold=True) -> None: # MB
...
def plot_joint_traj(qs: List[ndarray], title="joint_trj", hold=True): # MB
    qs_copy = np.array(qs)
    t = range(len(qs_copy))
...
def plot_cart_traj(poses: List[sm.SE3], title="poses_traj", conversion='rpy', convert=True, hold=True): # MB
    if conversion == 'rpy':
        n_subfigures = 6
        tcp_var = ["x", "y", "z", "r", "p", "y"]
    elif conversion == 'q':
...
if __name__ == "__main__":
    ...
    flat_rings = connect_rings(rings, connection_t=30, r=15)  # PER COLLEGARE "MEGLIO GLI ANELLI"

    """Init the tree from the poses we have and then save it"""
    jconf_tree = JconfTree(robot=robot, poses=flat_rings, transformations=transformations, n_confs_per_pose=20)
    jconf_tree.save("full_scan_new.json")
    ...
    df.to_csv('trj_new.csv', index=False)


    # Slice scan, arc tree
    #arc_trj = gen_arc_pose(obj_pose_ROBOT, radius=0.3, v_res=10) # PER CREARE LA SLICE TRAJECTORY
    arc_trj = gen_arc_poses(obj_pose_ROBOT, radius=0.3, v_res=10)  # PER CREARE LA SLICE TRAJECTORY # MB
    arc_tree = JconfTree(robot=robot, poses=arc_trj, transformations=transformations, n_confs_per_pose=20)
    arc_tree.save("arc_tree_new.json")

    trj, cost = arc_tree.gen_path([T.t for T in arc_trj]) # jconf_tree
    arc_trj_smooth = smooth_trj(trj, w_len=4, order=2)
    df = pd.DataFrame(arc_trj_smooth)
    df.to_csv('arc_trj_new.csv', index=False)

    # PER VISUALIZZARE LA TRAIETTORIA

    # LEGGENDO DA FILE
    # trj = []
    # with open('trj_smooth.csv', mode='r') as file:
    #     csv_reader = csv.reader(file)
    #     next(csv_reader)
    #     for row in csv_reader:
    #         trj.append(np.array([float(value) for value in row]))
    
    #cost = find_trj_cost(trj_smooth)
    # plot_joint_traj(trj_smooth, title=str(cost), hold=True) # Commented by MB since it gets stucked
